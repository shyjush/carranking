...
insert into public.car_models(manufacturer_id,name,category)
select mf.id,'EV4','전기 승용'
...
-- EV5 current base price is official. Keep dimensions/efficiency blank until the exact current specification page is verified.
...
insert into public.source_records(entity_type,entity_id,source_type,source_url,confidence,verified_at,notes)
select 'generation',g.id,'manufacturer','https://www.kia.com/kr/vehicles/ev4/specification','A',date '2026-08-31','EV4 current official specifications'
from public.generations g join public.car_models cm on cm.id=g.car_model_id join public.manufacturers mf on mf.id=cm.manufacturer_id
where mf.name='기아' and cm.name='EV4' and g.current=true
...
insert into public.source_records(entity_type,entity_id,source_type,source_url,confidence,verified_at,notes)
select 'generation',g.id,'manufacturer','https://www.kia.com/kr/vehicles/ev5/price','A',date '2026-08-31','EV5 current official price and Standard battery capacity'
from public.generations g join public.car_models cm on cm.id=g.car_model_id join public.manufacturers mf on mf.id=cm.manufacturer_id
where mf.name='기아' and cm.name='EV5' and g.current=true
