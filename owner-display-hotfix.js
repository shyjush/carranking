/* CarRanking owner display hotfix v5 */
(()=>{
'use strict';
...
async function dashboard(){
 const r=await fetch(`${BASE}/rest/v1/rpc/get_public_owner_dashboard`,{method:'POST',headers:H,body:'{}',cache:'no-store'});
 const t=await r.text();if(!r.ok)throw new Error(`owner dashboard ${r.status} ${t}`);return t?JSON.parse(t):{};
}
async function refreshOwnerDisplay(){
 try{
  const d=await dashboard(),ranking=Array.isArray(d.ranking)?d.ranking:[],reviews=Array.isArray(d.reviews)?d.reviews:[],sum=Number(d.rating_count||0);
  ...
  if(feed)feed.innerHTML=reviews.length?reviews.map(review).join(''):'<div class="empty"><strong>첫 오너 리뷰를 남겨주세요.</strong><p>아직 공개된 리뷰가 없습니다.</p></div>';
  window.CR_OWNER_REFRESHED={sum,ranking:ranking.length,reviews:reviews.length,ts:Date.now()};
 }catch(e){console.warn('owner dashboard refresh failed',e)}
}
function boot(){refreshOwnerDisplay();[400,1200,3000,7000].forEach(ms=>setTimeout(refreshOwnerDisplay,ms));window.addEventListener('cr-review-saved',()=>setTimeout(refreshOwnerDisplay,150));document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshOwnerDisplay()});window.addEventListener('pageshow',refreshOwnerDisplay)}
